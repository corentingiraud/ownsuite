...
Pluggable (ADR-003): `external` (a managed EU S3 — files live off the box) or `in-cluster`
(a single-node RustFS store, deployed for you — ADR-046). With a `provider`
in external mode, `suite apply` creates the buckets for your enabled apps; in `in-cluster`
mode RustFS's bucket-init Job creates them.
...
!!! tip "`external` needs a CORS-capable S3"
    ...
    `in-cluster` instead — it proxies media same-origin.
