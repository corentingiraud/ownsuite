...
- [x] Mailbox: **suitenumerique/messages** — La Suite's own mail app (Postfix MTA-in + Django MDA + Postgres/Redis + integrated webmail, no IMAP), federated to the same Keycloak via OIDC; local chart, off by default (ADR-021, ADR-026). **OpenSearch deferred** (optional upstream; protects single-VPS RAM)
- [x] Outbound via `MTA_OUT_MODE=relay` through a reputable EU SMTP relay (Infomaniak) — never direct from the VPS IP
- [x] MX/SPF/DKIM/DMARC in the DNS flow (DKIM key generated by the installer); inbound SMTP port 25 via ServiceLB (ADR-027); rDNS/PTR documented as a host step; mailbox provisioned by the maildomain seed Job + JIT (no `suite user add` change)
- [ ] **Off-CI gates (not yet run):** the hermetic loopback boot on a beefier runner, and the human deliverability check (inbox-not-spam) on a real domain + relay — like Grist/Projects, messages is template/lint-validated, not CI-booted (runner memory)
- **DoD:** `suite user add` also creates the mailbox; outbound mail reaches the inbox (not spam) — built and lint-validated; the inbox-not-spam half is the remaining off-CI human check.
